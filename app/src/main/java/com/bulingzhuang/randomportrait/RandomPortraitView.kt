...
class RandomPortraitView : View {
    ...
    init {
        ...
        mBorderWidth = 0
        mBorderPaint = Paint()
        mPortraitPaint = Paint()
        mHalfStr = ""
        mCenterStr = ""
        color = ""
        mPortraitCode = ""
        mCallback = null
        colorList = listOf("#f69988", "#f36c60", "#e84e40", "#e51c23", "#dd191d", "#d01716", "#c41411",
    ...
    fun setCode(code: String) {
        if (code.length == 22) {
            mHalfStr = code.substring(0, 10)
            mCenterStr = code.substring(10, 15)
            color = code.substring(15, code.length)
    ...
    fun randomX3(): List<String> {
        val list = ArrayList<String>()
        for (index in 0..2) {
            val random = Random()
            val halfPieceNum = random.nextInt(1024)
            val centerPieceNum = random.nextInt(32)
            val color = colorList[random.nextInt(133)]
            var halfStr = Integer.toBinaryString(halfPieceNum)
            val lenH = 10 - halfStr.length
            for (position in 1..lenH) {
                halfStr = "0" + halfStr
            }
            var centerStr = Integer.toBinaryString(centerPieceNum)
            val lenC = 5 - centerStr.length
            for (position in 1..lenC) {
                centerStr = "0" + centerStr
            }
            val portraitCode = halfStr + centerStr + color
            list.add(portraitCode)
            Log.e("blz", "第$index 个代码是：$portraitCode")
        }
        return list
    }
        ...
        if (width > 0 && height > 0) {
            ...
            for (index in mHalfStr.indices) {
                when (mHalfStr[index]) {
                    '1' -> {
                        ...
                        canvas.drawRect(pieceHeight * columns - 0.5f, pieceHeight * rows - 0.5f, pieceWidth * (columns + 1) + 0.5f, pieceHeight * (rows + 1) + 0.5f, piecePaint)
                        canvas.drawRect(pieceHeight * (4 - columns) - 0.5f, pieceHeight * rows - 0.5f, pieceWidth * (4 - columns + 1) + 0.5f, pieceHeight * (rows + 1) + 0.5f, piecePaint)
                    }
                }
            }
            ...
            for (index in mCenterStr.indices) {
                when (mCenterStr[index]) {
                    '1' -> {
                        canvas.drawRect(pieceHeight * 2 - 0.5f, pieceHeight * index - 0.5f, pieceWidth * 3 + 0.5f, pieceHeight * (index + 1) + 0.5f, piecePaint)
                    }
                }
            }
